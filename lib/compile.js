...
module.exports = function(config, fn){
  derp.sync(config, module.exports.download, module.exports.extract,
    module.exports.embed, module.exports.icon, module.exports.make,
    module.exports.deliver, fn);
};

module.exports.icon = function(config, fn){
  var debug = require('debug')('lone:compile:icon');

  if(!config.icon){
    debug('no icon specified in config');
    return fn(null, config);
  }

  if(!isWindows){
    debug('not windows so icon is moot');
    return fn(null, config);
  }
  ...
  fs.readFile(rc, 'utf-8', function(err, data){
    ...
    var patch = dmp.patch_make(data, data.replace(/1 ICON .*/, '1 ICON ' + config.icon));
    if(patch.length === 0){
