...
module.exports.download = function(config, fn){
  var debug = require('debug')('lone:source:download');
...
module.exports.extract = function(config, fn){
  var debug = require('debug')('lone:source:extract'),
  ...
  fs.exists(config.node.working, function(exists){
    ...
    debug('ungzip-ing...');
    input.pipe(ungzip);
    ungzip.on('error', fn).on('finished', function(){
      debug('ungzipped');
    });
...
module.exports.embed = function(config, fn){
  var debug = require('debug')('lone:source:embed'),
...
module.exports.make = function(config, fn){
  ...
  fs.exists(config.node.out, function(exists){
    ...
    derp.sync(config,
      ...
      // derp.run('/usr/bin/strip ' + config.node.out + ' -o ' + config.node.out),
      function(err, config){
        if(err) return fn(err);
