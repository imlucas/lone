...
function argvPatch(fn){
  ...
  fs.readFile(src, 'utf-8', function(err, contents){
    ...
    if(contents.indexOf('const char *argv0 = argv[0];') > -1){
      return fn();
    }
    var search = {
        // v_0_10_0: 'static void ParseArgs(int argc, char **argv) {',
        v_0_11_0: 'const char*** exec_argv) {'
      },
      patch = 'if(argc > 0){const char *argv0 = argv[0]; *(--argv) = argv0; argc++;}\n';

    // if(contents.indexOf(search.v_0_10_0) > -1){
    //   contents = contents.replace(search.v_0_10_0,
    //     search.v_0_10_0 + '\n' + patch);
    // }
    if(contents.indexOf(search.v_0_11_0) > -1){
      contents = contents.replace(search.v_0_11_0,
        search.v_0_11_0 + '\n' + patch);
    }
...
function make(fn){
  ...
  fs.exists(config.node.out, function(exists){
    ...
    if(isWindows) return run('cmd /c vcbuild.bat nosign nosnapshot release')(fn);
